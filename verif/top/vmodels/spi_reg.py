...
class HamsterSPIInterface:
    ...
    def _setup_spi(self) :
        ...
        self.spi_mon_so = SPIMonitor(SerialMode.MASTER,self._itf)
        self.spi_mon_mo = SPIMonitor(SerialMode.SLAVE,self._itf)
        self.spi_mon_so.spi_mode = spi_mode
        self.spi_mon_mo.spi_mode = spi_mode
    ...
    def _start_monitoring(self) :
        ...
        async def mon_slave():
            while True :
                ...
                log.info(f"    Recieved word {self.last_read_value}")
                self.got_word.set()
    ...
    def compress_write(self) :
        ...
        for req in self.pending_accesses :
            ...
            if isinstance(target, Field) :
                ...
                log.error(f"Invalid request payload type :{type(target)!s}")
                continue
    ...
    async def send_write(self) :
        ...
        for req in self.pending_accesses :
            ...
            if previous_reg is None or previous_reg.offset != current_reg.offset -1 :
                ...
                await self.spi_idle()
                cocotb.log.info(f"Start write transaction to {current_reg.name}")
                if previous_reg is not None :
                    cocotb.log.info(f"Delta offset is {current_reg.offset} - 1 = {previous_reg.offset}")
                if isinstance(req,WriteRequest) :
                    self.store_sof_write(current_reg.offset)
            else :
                cocotb.log.info(f"Continue write on {current_reg.name}")
